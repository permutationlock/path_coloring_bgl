...
{
	// Let t_i be the last vertex added to T, and s_0 the last interior vertex
	...
	while(edge_iter++ != neighbor_range_map[t_i].second)
	{
		...
		if(continue_path || color_map[n] == p_color) {
			...
			while(edge_iter++ != neighbor_range_map[t_i].second) {
				...
				// If v is in P and l is uncolored, we have vertices to color
				if(color_map[v] == p_color)
				{
						...
						&& color_map[l] != t_color)
					{
						poh_color_recursive(
								...
								neighbor_range_map, l, face_mark, count,
								q_color, p_color, t_color
							);
					}
				}
