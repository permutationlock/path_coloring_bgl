...
\end{tikzpicture}
\hfill\\
\textbf{Figure 2.1} The case of a $t_0t_1$-path.
\end{center}
...
\end{tikzpicture}
\hfill\\
\textbf{Figure 2.2} The case of no $t_0t_1$-path.
\end{center}
...
\begin{tikzpicture}[
  ...
  \node (p) [label=above:{$c_0$}] at (0cm, 2cm) {};
...
$L_0(v)$ satisfies the inductive hypothesis with $G_0$, $x_0=c_n$, $y_0=c_i$, and $p_0$. After choosing $G_0$
...
\noindent Suppose $y=p$, $x\ne p$, and $c_i\not\in V(C[x,p))$. Since $\alpha\not\in L(v)$
for any $v\in V(C[x,p))$ we precolor $x$ from $L_0(x)$ and apply the inductive hypothesis to choose
...
\end{tikzpicture}
\hfill\\
\textbf{Figure 3.5} The case $x=p$, $y\ne p$, and $c_i\in V(C(y,x))$.
\end{center}

\noindent Suppose $x=p$, $y\ne p$, and $c_i\in V(C(y,x))$. In this case $G_1$ must exist, so first apply the
...
\end{tikzpicture}
\hfill\\
\textbf{Figure 3.6} The case $x=p$, $y\ne p$, and $c_i\not\in V(C(y,x))$ (shown is the case $y\ne c_i$ and
$\alpha\in L(c_i)$).
\end{center}
...
\noindent Suppose $x\ne p$, $y\ne p$, and $c_i\in V(C[x,p))$. In this case $G_1$ must exist, so first apply
the inductive hypothesis to choose $G_1$ from $L$ with $x_1=x$, $y_1=y$, and $p_1=p$.
Since $\alpha\not\in L(v)$ for any $v\in V(C[x,p))$ we may apply the inductive hypothesis to choose $G_0$ from
...
\noindent Suppose $x\ne p$, $y\ne p$, and $c_i\in V(C(y,x))$. In this case $G_1$ must exist. We apply
...
\begin{tikzpicture}[
  ...
  \node (p) [label=above:{$x_1$, $p_1$}] at (0cm, 2cm) {};
  \node (p_label) [draw=none, scale=0.8] at (0cm, 2cm) {$\alpha$};
  \node (c_i) [label=below:$y_1$, fill] at (0cm, -1cm) {};
  \node (G_1) [draw=none] at (0.5cm, 0.5cm) {$G_1$};
  \node (null) [draw=none] at (270:1.5cm) {};
  \draw (p) edge (c_i);
  \draw (p) edge [bend left=90] (c_i) [dashed];
