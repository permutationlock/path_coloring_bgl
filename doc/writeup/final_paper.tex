...
\begin{figure}
\begin{tikzpicture}
  \node (v0) [label=above:$0$] at (0cm, 3cm) {};
  \node (v1) [label=below left:$1$] at (-1.5cm, 0cm) {};
  \node (v2) [label=below right:$2$] at (1.5cm, 0cm) {};
  \node (v3) [label=above:$3$] at (-0.75cm, 0cm) {};
  \node (v4) [label=above:$4$] at (0.75cm, 0cm) {};
  
  \begin{pgfonlayer}{bg}
	  \draw (v0) edge (v1); \draw (v0) edge (v2); \draw (v2) edge (v1);
	  \draw (v0) edge (v3); \draw (v1) edge (v3); \draw (v0) edge (v4);
	  \draw (v2) edge (v4); \draw (v3) edge (v4);
  \end{pgfonlayer}
  
\end{tikzpicture}
\caption{A drawing of the graph constructed in \ref{example_make_graph}.}
\end{figure}
