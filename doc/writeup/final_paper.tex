...
line segments between points such that no lines cross.
...
An $n$-vertex \textit{path} consists of the vertices $v_1,v_2,\ldots,v_n$ and the edges
$v_1v_2,$ $v_2v_3,$ $\ldots,$ $v_{n-1}v_n$. A length $n$ \textit{cycle}, or $n$-cycle, consists of
...
\caption{Drawings of $K_3$, $K_4$, $K_5$ (nonplanar), a $4$-vertex path, and a
	$6$-cycle.}
\end{center}
\end{figure}
...
independently proved the conjecture. Planar graphs that do not admit a path $2$-coloring
...
A breadth first search requires at most $2m$ lookups. Moreover, the vertex will
$u$ will be colored following the search. Thus we perform at most one breadth
first search from each vertex, requiring at most $2nm$ operations.
Therefore the complexity of the algorithm is $\mathcal{O}(2m + 2nm)
=\mathcal{O}(n^2)$.
...
$\text{Adj}[v]$. Note that the reference $r_v$ may be located in $\mathcal{O}(1)$
...
\noindent\textbf{Output:} A path $L$-list-coloring of $\text{Int}(C)$ such that $x$ and
$y$ each share a color with at most one of their neighbors, and no vertex in
$\text{Int}(C)-P$ with a neighbor in $P$ receives the color $c$. If $x=y$ then no neighbor
of $x$ in $\text{Int}(C)$ will receive the same color as $x$.
...
defined in figure \ref{augmented_notation} and valid expressions for these objects are
...
\texttt{embedding} & an object of type \texttt{embedding\_t}\\
...
of $2$ or more colors, and each vertex interior to the cycle a sequence of
$3$ or more colors.
...
In this project we considered two recursive procedures on plane graphs: one
