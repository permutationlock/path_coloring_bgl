...
Let the initial $2$-coloring of the outer triangle of $G_k$ assing the top
...
\]
More specificaly, it is $\Theta(n)$.
...
The Hartman-Skrekovski algorithm decribed in the previous section relied on the
...
\section{Path Coloring in C++ with the Boost Graph Library}
...
discuss implementing the Poh and Hartman-Skrekovski using BGL abstractions.

We will assume familiarity with the C++ language and the
\verb!C++! Standard Template Library (STL). Full hyperlinked documentation is
available with the project source code at
\url{https://github.com/permutationlock/path_coloring_bgl},
with links to the relevent Boost and STL concepts.

\subsection{The Boost Graph Library}

The BGL provides several abstract concepts for graph data structures.
The basic \verb!Graph! concept requires that a vertex and edge type to be
defined, as well as a few properties such as whether the graph is directed or
undirected. The \verb!VertexListGraph! and \verb!EdgeListGraph! refine this
concept to additionally require an interface to iterate over the vertex and
edge sets, respectively. The \verb!VertexAndEdgeListGraph! concept simply
combines the two refinements.

Although other concepts exist such as
\verb!AdjacencyGraph!, we will only require that input graphs model
\verb!VertexListGraph! or \verb!VertexAndEdgeListGraph!. Although
\verb!AdjacencyGraph! might seem like an obvious choice, we will follow the
BGL's decision and represent the rotation scheme for planar embeddings as an
exterior property map.
...
assigned via \verb!get! or \verb!put! functions, respectively, on the graph
structure itself. Exterior properties are properties stored in a separate data
structure. The structure then allows \verb!get! and/or \verb!put! operations
to allow reads and/or writes to individual vertex or edge properties.

We will use exterior property maps that satisfy the
\verb!LvaluePropertyMap! concept. The \verb!LvaluePropertyMap! concept requires
that the return value for \verb!get! calls on the property map be a reference.
The BGL defines the \verb!PlanarEmbedding! concept to refine
\verb!LvaluePropertyMap! to require that each vertex is assigned a range of
edges, representing the embedding ordered rotation scheme.

\subsection{Poh with BFS in the BGL}

Here we describe our BGL implementation of \textit{Poh with BFS} (4.2). The
publicly visible function is as follows.\\
...
The function makes the following restrictions on template types.\\
\begin{center}
\verb!vertex_t! is  \verb!boost::graph_traits<graph_t>::vertex_descriptor!\\
\hfill\\

\begin{tabular}{l|l|l}
Type & Concept & Additional Requirements\\
\hline
\verb!graph_t! & \verb!VertexAndEdgeListGraph! & None\\
\verb!color_t! & \verb!EqualityComparable!, & None\\
& \verb!CopyAssignable! & \\
\verb!planar_embedding_t! & \verb!PlanarEmbedding! & None\\
\verb!color_map_t! & \verb!LvaluePropertyMap! & \verb!value_type! is \verb!color_t!\\
\verb!mark_map_t! & \verb!LvaluePropertyMap! & \verb!value_type! is an integer\\
\verb!parent_map_t! & \verb!LvaluePropertyMap! & \verb!value_type! is \verb!vertex_t!\\
\verb!vertex_iterator_t! & \verb!InputIterator! & \verb!value_type! is \verb!vertex_t!
\end{tabular}
\end{center}
\hfill\\
...
Finally we assume the two ranges of verteces map are paths in the graph
satisfying the requirements of \textit{Poh with BFS} (4.2).
We assume no vertex is already colored any of \verb!c_0!, \verb!c_1!,
or \verb!c_2!. We also require that all vertices are initially assigned a mark
of $0$ by \verb!mark_map!.

The implementation almost directly follows the description of (4.2), although
some decisions had to be made. In order
to keep the algorithm linear, we must ensure we iterate through the
adjacency list of a given vertx precisely once during the orientation phase of
...
Source code is available online at the link provided earlier.
...
\bibitem{reed}
	...
	1987.
\bibitem{boost}
	Siek, J., Lee, L., Lumsdaine, A., \textit{The Boost Graph Library: User
