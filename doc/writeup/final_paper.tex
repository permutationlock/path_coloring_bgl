...
planar graph together with a planar embedding is a \textit{plane graph}. In a
plane graph we shall assume all edge curves are line segments, which is
allowable due to the following result by F\'ary.
...
We define the collection of graphs $\{G_k \ : \ k\in\mathbb{N}\}$, depicted in \ref{poh_exmpl}.
...
Recall however, that the correctness of Poh's algorithm did not require that $T$
...
to color any unocolored vertices in $\text{Int}(C_1)$.
 ...
 Let
$T=t_1t_2\ldots t_d$ be the path constructed above.
