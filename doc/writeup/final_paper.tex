...
\tikzset{every picture/.append style={scale=1.2}}
...
based on Hartman's proof and show it runs in $\mathcal{O}(n)$ time.
...
\begin{tikzpicture}
  ...
  \node (u) [label=below:$t_0$] at (-1.5cm, 0cm) {};
...
than the description given by both Hartman and Skrekovski. This restructuring is, in
...
We will call $x$ and $y$ fixed vertices and assume all
vertices are uncolored except for potentially $x$ and $y$. If $x$ or $y$ are
colored, assume $L[x]$, $L[y]$ contain only the color they have been assigned.
...
until we hit neighbor $v_i\in C$, $i\ne k$. Define the cycles $C_1$ and $C_2$
as in (5.1). At this point we will have removed $v_1$ from the $\text{Int}(C_1)$
by setting up each neighbors of $v_1$ as an outer face vertex. We will color
$\text{Int}(C_1)-v_1$ accordingly with \textit{Path Color} (5.3). Then we will
color $\text{Int}(C_2)$ with \textit{Remove Path} (5.4).

Let us iterate counterclockwise through $\text{Adj}[v_1]$ beginning from $v_k$.
Let $u$ be the current neighbor of $v_1$.

\textit{Case 1:} Suppose $u\not\in C$. After removing $v_1$, $u$ will be on the
outer face. Thus set the state of $u$ to indicate $u$ is
on the outer face. Additionally, construct $\text{Nbr}[u]=(r_1,r_2)$ such that
...
\textit{Case 2.2:} Suppose $u\ne v_k$. Let $\text{Nbr}[u]=(r_1,r_2)$ and
$\text{Nbr}[u]=(s_1,s_2)$. Let $r_v$ be the reference to the node for
$v$ in $\text{Adj}[u]$ and $s_u$ be the reference to the node for $u$ in
$\text{Adj}[v]$. There are several cases to consider

\textit{Case 2.2.1:} Suppose $u\in C[x,v_1]$. 

\textit{Case 2.2.2:} Suppose $u\in C[y,x]$.

\textit{Case 2.2.3:} Suppose $u\in C[v_1,y]$.  
