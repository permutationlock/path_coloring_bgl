...
\texttt{C++} Standard Template Library (STL). Full hyperlinked documentation is
...
\texttt{VertexListGraph} or \texttt{VertexAndEdgeListGraph}. Although
\texttt{AdjacencyGraph} might seem like an obvious choice, we will follow the
...
Here we describe our BGL implementation of \textit{Poh -- BFS} (4.2). The
...
\end{tabular}
\end{center}
\caption{Typedefinitions for Poh with BFS.}
\end{figure}
...
\hline
\texttt{graph\_t} & \texttt{VertexAndEdgeListGraph} & None\\
...
\texttt{mark\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is an integer\\
\texttt{parent\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is \texttt{vertex\_t}\\
...
Finally we assume the two ranges of vertices map are paths in the graph
satisfying the requirements of \textit{Poh -- BFS} (4.2).
We assume no vertex is already colored any of \texttt{c\_0}, \texttt{c\_1},
or \texttt{c\_2}. We also require that all vertices are initially assigned a mark
of $0$ by \texttt{mark\_map}.
...
\subsection{Poh with Path Walking in the BGL}

Here we describe our BGL implementation of the linear time Poh algorithm
...
template<
        ...
        typename color_map_t,
        typename mark_map_t,
        typename n_range_map_t,
        typename vertex_iterator_t,
        typename color_t
    >
void poh_color(
        const graph_t & graph,
        const embedding_t & planar_embedding,
        color_map_t & color_map,
        n_range_map_t & neighbor_range_map,
        mark_map_t & mark_map,
        vertex_iterator_t p_begin, vertex_iterator_t p_end,
        vertex_iterator_t q_begin, vertex_iterator_t q_end,
        color_t c_0, color_t c_1, color_t c_2
...
\hline
\texttt{vertex\_t} &  \texttt{boost::graph\_traits<graph\_t>::vertex\_descriptor}\\
\texttt{n\_iter\_t} & \texttt{boost::property\_traits<embedding\_t>::value\_type}\\
& $\qquad$\texttt{::const\_iterator}\\
\texttt{n\_range\_t} & \texttt{std::pair<n\_iter\_t,n\_iter\_t>}
\end{tabular}
\end{center}
\caption{Typedefinitions for Poh with Path Walking.}
\end{figure}
...
\texttt{color\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is \texttt{color\_t}\\
\texttt{mark\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is an integer\\
\texttt{n\_range\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is \texttt{n\_range\_t}\\
\texttt{vertex\_iterator\_t} & \texttt{InputIterator} & \texttt{value\_type} is \texttt{vertex\_t}
\end{tabular}
\end{center}
...
about input structures as in our implementation of \textit{Poh -- BFS} (4.2) in
the previous section.

The implementation follows algorithms (4.3) and (4.4) with the following
implementation decisions an modifications. We use the
\texttt{neighbor\_range\_map} structure to track start and stop points in the cyclic
...
template<
        ...
        typename n_range_map_t,
        typename location_map_t,
        typename face_iterator_t
    >
void hartman_skrekovski_color(
        const graph_t & graph,
        const embedding_t & augmented_embedding,
        color_list_map_t & color_list_map,
        n_range_map_t & neighbor_range_map,
        location_map_t & face_location_map,
...
\end{tabular}
\end{center}
\caption{Typedefinitions for Hartman-Skrekovski.}
\end{figure}
...
\texttt{location\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is an integer\\
\texttt{n\_range\_map\_t} & \texttt{Lvalue{\allowbreak}Property{\allowbreak}Map} & \texttt{value\_type} is \texttt{n\_range\_t}\\
\texttt{vertex\_iterator\_t} & \texttt{InputIterator} & \texttt{value\_type} is \texttt{vertex\_t}
\end{tabular}
\end{center}
...
the graph.
We assume the range of vertices is a cycle in the provided graph.
We assume \texttt{color\_list\_map} assigns each vertex in the cycle a sequence
of at $2$ or more colors, and each vertex interior to the cycle a sequence of
$3$ or more colors. We also require that all vertices are initially assigned a
location mark of $0$ by \texttt{location\_map}.
...
representations. Finally, we showed each procedure admits an algorithm that
runs in linear time.
...
The combined may then be adapted to a path $4$-coloring of the original torus
graph. It would be interesting to see if this procedure might be adapted to
